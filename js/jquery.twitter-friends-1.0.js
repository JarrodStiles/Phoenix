...
$.fn.twitterFriends = function(allOptions) {  
	...
	return this.each(function() {  
		...
		var addUsers= function() {
			...
			//usrsDiv.html(''); //removes all users from display
			//next visible page
			vp++;
			//Add 
		...
		var addUser = function(x,i) {
			$(getUserHTML(x,i)).appendTo(usrsDiv);
		};
		
		var swapUser = function(x,i) {
			...
			$x.replaceWith($('<a style="display:none;height:'+op.user_image+'px" href="'+u+'" title="'+ t +'">'
				+'<img src="'+x.profile_image_url+'" border="0" height="'+op.user_image+'" width="'+op.user_image+'"/>'
			+'</a>'));
		};
		
		var getUserHTML = function(x,i) {
			var u = op.user_link && x.url? x.url :'http://twitter.com/' +x.screen_name;
			var t = x.name + (x.status && op.tweet ?': '+ x.status.text :'');
			t= t.replace(/"/g,'&quot;').replace(/'/g,'&#39;');
			user = '<a style="display:none;height:'+op.user_image+'px" href="'+u+'" title="'+ t +'">'
				+'<img src="'+x.profile_image_url+'" border="0" height="'+op.user_image+'" width="'+op.user_image+'"/>'
			+'</a>';			
			return user;
		};
